...
TEST_F(TradeApiTest, PlaceOrderWritesOrderReq) {
    const DzOrderId id = dz_place_order("CTP001", "IF2401", DZ_DIRECTION_LONG, DZ_PRICE_LIMIT,
    ...
    EXPECT_STREQ(req->strategy_id, dz_strategy_id());
...
TEST_F(TradeApiTest, CancelOrderWritesOrderCancelReq) {
    ASSERT_TRUE(dz_cancel_order("CTP001", 42));
    const auto* req =
...
TEST_F(TradeApiTest, SetLogicalPositionWritesFrame) {
    ASSERT_TRUE(dz_set_logical_position("CTP001", "IF2401", 3));
    const auto* pos =
    ...
    EXPECT_STREQ(pos->strategy_id, dz_strategy_id());
    EXPECT_EQ(pos->net_volume, 3);
}

// ── 失败路径: 未初始化守卫 ──

TEST_F(TradeApiTest, PlaceOrderNotInitializedReturnsError) {
    dz_release();
    EXPECT_EQ(dz_place_order("CTP001", "IF2401", DZ_DIRECTION_LONG, DZ_PRICE_LIMIT, 1, 1,
                             DZ_POSITION_EFFECT_OPEN),
              -1);
    EXPECT_EQ(dz_errcode(), DZ_EC_STRATEGY_NOT_INITIALIZED);
    EXPECT_NE(dz_errmsg()[0], '\0');
}

TEST_F(TradeApiTest, CancelOrderNotInitializedReturnsError) {
    dz_release();
    EXPECT_FALSE(dz_cancel_order("CTP001", 42));
    EXPECT_EQ(dz_errcode(), DZ_EC_STRATEGY_NOT_INITIALIZED);
}

TEST_F(TradeApiTest, SetLogicalPositionNotInitializedReturnsError) {
    dz_release();
    EXPECT_FALSE(dz_set_logical_position("CTP001", "IF2401", 3));
    EXPECT_EQ(dz_errcode(), DZ_EC_STRATEGY_NOT_INITIALIZED);
}
