// 03-process.md 契约类型层测试：帧号、枚举、结构体、校验
...
// --- 帧号与帧头布局（契约第 5 行：四个帧均使用 DzExtFrameHeader，无 instance_id）---
...
// --- 枚举序列化（契约第 13-48 行；PascalCase 字符串）---
...
// --- ProcessConfig 全量角色（契约第 75-87 行）---
...
TEST(ProcessTest, ProcessConfigOmitsEmptyDisplayName) {
    // display_name 空串与 nullopt 均省略（RTN wire 等价，契约第 79 行"缺失=未设置"）
    ProcessConfig c;
    c.args = {};
    c.env = {};
    c.restart = {};
...
TEST(ProcessTest, ProcessConfigRequiresFullFields) {
    // 全量角色 args/env/restart 必须出现（契约第 79 行），缺失抛异常
...
// --- ProcessControlReq（契约第 101-125 行）---
...
TEST(ProcessTest, ProcessStatusOmitsEmptyAndNullopt) {
    // 空串省略（契约第 185 行"缺省为空串"）；event 缺失 = 自发状态变化
...
// --- SetProcessConfigReq（契约第 193-228 行，payload 命名见设计文档）---
...
// --- 校验：增量角色 validate_process_config_patch（契约第 70-73、217-228 行）---
...
// --- 校验：全量角色 validate_process_config_full（契约第 75-79 行）---
...
TEST(ProcessTest, FromJsonRequiresRequiredFields) {
    // ProcessControlReq: action/target 必填（契约第 107-110 行）
    ...
    EXPECT_THROW((nlohmann::json{{"action", "Start"}}).get<ProcessControlReq>(),
                 nlohmann::json::exception);
    // ProcessStatus: name/state/pid 必填（契约第 151-155 行）
    EXPECT_THROW((nlohmann::json{{"state", "Running"}, {"pid", 1}}).get<ProcessStatus>(),
                 nlohmann::json::exception);
    // SetProcessConfigReq: target 必填（契约第 199-200 行；config 缺失已在
...
// --- apply_process_config_patch（契约第 80-87 行选择性合并）---
...
TEST(ProcessTest, ApplyPatchOverridesArgsAndRestart) {
    ...
    EXPECT_EQ(merged["args"], nlohmann::json::array({"-x"}));   // args 整体覆盖（契约第 84 行）
    EXPECT_EQ(merged["restart"]["enabled"], false);             // restart 整体覆盖（契约第 86 行）
    EXPECT_EQ(merged["restart"], patch["restart"]);             // 原子替换：结果与 patch 完全一致（契约第 86 行整体覆盖）
...
TEST(ProcessTest, ApplyPatchMergesEnvAndDeletesWithNull) {
    ...
    auto merged = apply_process_config_patch(current, patch);
    EXPECT_EQ(merged["env"]["A"], "1");                // 保留（契约第 85 行递归合并）
...
TEST(ProcessTest, ApplyPatchDisplayNameSemantics) {
    ...
    EXPECT_EQ(m1["display_name"], "CTP行情");           // 覆盖（契约第 87 行）
