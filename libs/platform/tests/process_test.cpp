...
// --- 帧号与帧头布局（契约 04-process：四个帧均使用 DzExtFrameHeader，无 instance_id）---
...
// --- 枚举序列化（契约 04-process；PascalCase 字符串）---
...
TEST(ProcessTest, RestartPolicySerializes) {
    ...
    // 全量角色：三字段必填（契约 04-process），缺失任一抛异常
...
// --- ProcessConfig 全量角色（契约 04-process）---
...
TEST(ProcessTest, ProcessConfigOmitsEmptyDisplayName) {
    // display_name 空串与 nullopt 均省略（RTN wire 等价，契约 04-process"缺失=未设置"）
    ProcessConfig c;
    c.args = {};
    c.env = {};
    c.restart = {};
...
TEST(ProcessTest, ProcessConfigRequiresFullFields) {
    // 全量角色 args/env/restart 必须出现（契约 04-process），缺失抛异常
...
// --- ProcessControlReq（契约 04-process）---
...
TEST(ProcessTest, ProcessStatusOmitsEmptyAndNullopt) {
    // 空串省略（契约 04-process"缺省为空串"）；event 缺失 = 自发状态变化
...
// --- SetProcessConfigReq（契约 04-process，payload 命名见设计文档）---
...
// --- 校验：增量角色 validate_process_config_patch（契约 04-process）---
...
TEST(ProcessTest, ValidatePatchRejectsBadRestart) {
    // restart 三子字段必须全部出现（契约 04-process）
...
// --- 校验：全量角色 validate_process_config_full（契约 04-process）---
...
TEST(ProcessTest, FromJsonRequiresRequiredFields) {
    // ProcessControlReq: action/target 必填（契约 04-process）
    ...
    EXPECT_THROW((nlohmann::json{{"action", "Start"}}).get<ProcessControlReq>(),
                 nlohmann::json::exception);
    // ProcessStatus: name/state/pid 必填（契约 04-process）
    EXPECT_THROW((nlohmann::json{{"state", "Running"}, {"pid", 1}}).get<ProcessStatus>(),
                 nlohmann::json::exception);
    // SetProcessConfigReq: target 必填（契约 04-process；config 缺失已在
...
// --- apply_process_config_patch（契约 04-process选择性合并）---
...
TEST(ProcessTest, ApplyPatchOverridesArgsAndRestart) {
    ...
    EXPECT_EQ(merged["args"], nlohmann::json::array({"-x"}));   // args 整体覆盖（契约 04-process）
    EXPECT_EQ(merged["restart"]["enabled"], false);             // restart 整体覆盖（契约 04-process）
    EXPECT_EQ(merged["restart"], patch["restart"]);             // 原子替换：结果与 patch 完全一致（契约 04-process整体覆盖）
...
TEST(ProcessTest, ApplyPatchMergesEnvAndDeletesWithNull) {
    ...
    auto merged = apply_process_config_patch(current, patch);
    EXPECT_EQ(merged["env"]["A"], "1");                // 保留（契约 04-process递归合并）
...
TEST(ProcessTest, ApplyPatchDisplayNameSemantics) {
    ...
    EXPECT_EQ(m1["display_name"], "CTP行情");           // 覆盖（契约 04-process）
