...
bool is_integer_value(const nlohmann::json& j) {
    // 契约第 82 行:整数字段不区分 int32/int64/uint64,JSON 数字只要值在目标类型范围内即可
...
nlohmann::json ShmConfigBase::merge_patch(const nlohmann::json& patch) const {
    // 契约第 73 行:payload 非 object 为校验失败
    if (!patch.is_object()) {
    ...
    auto apply_int_field = [&patch](nlohmann::json& m, const char* key) {
        ...
        // 契约第 71 行:除 preload_points 内部 key 的 value 外,null 均校验失败
        if (v.is_null()) {
    ...
    if (patch.contains("preload_points")) {
        ...
        // preload_points 本身不允许 null(契约第 71 行)
        if (pp.is_null()) {
        ...
        for (auto it = pp.begin(); it != pp.end(); ++it) {
            ...
            if (val.is_null()) {
                ...
                // 契约第 77 行:value 非 object 且非 null 为校验失败
...
void ShmConfigBase::set_shm_config(const nlohmann::json& patch) {
    ...
    // 契约第 80 行:空对象 {} 视为无操作。值无变化(含 {}、page_size_mb 单独提交、
