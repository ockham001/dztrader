...
dztrader::shm::NamedSemaphore* g_event_sem = nullptr;

// 信号处理: SIGTERM/SIGINT 触发优雅退出
// 仅调用异步信号安全操作: atomic store + notify (sem_post 封装)
extern "C" void signal_handler(int sig) {
    g_shutdown_requested.store(true, std::memory_order_relaxed);
    if (g_event_sem != nullptr) {
        g_event_sem->notify();  // 唤醒主循环的 event_queue_->wait()
    }
...
int main(int argc, char* argv[]) {
    ...
    // 重命名 exe (如 dztd_ctp_2) 即自动作为新网关, 无需命令行参数。
    ...
    //    log_config_ 在 TdApi 构造体内从 cfg_path 的 "log" section 自行加载 (失败用默认值自愈)
    nlohmann::json log_json;
    try {
        log_json = dztrader::core::load_json_section<nlohmann::json>(cfg_path, "log");
    } catch (const std::exception& e) {
        spdlog::error("log config load failed | error=\"{}\"", e.what());
        return 1;
    }
    ...
    // 2. 加载 td section (失败 fatal)
    TdConfig td_cfg;
    try {
        td_cfg = TdConfig::load(cfg_path, "td");
    } catch (const std::exception& e) {
        SPDLOG_CRITICAL("config load failed | error=\"{}\"", e.what());
        return 1;
    }

    if (auto err = validate(td_cfg)) {
    ...
    // dztd_ctp 的 reader_name 必须与之一致, master 的 notify_subscribers 才能 notify 到.
    ...
    g_event_sem = event_sem.get();
    std::signal(SIGTERM, signal_handler);
    std::signal(SIGINT, signal_handler);

    // 5. 构造 event_queue (多账户 SPI 线程 -> 主线程, MPMC 共用 event_sem)
    auto event_queue = std::make_shared<MpmcQueue>(event_sem.get(), 4096);

    // 6. 构造 TdApi (内部创建 event reader/writer + OrderIdMeta + PersistWriter, 构造时加载 log_config_)
    std::unique_ptr<TdApi> api;
    try {
        api = std::make_unique<TdApi>(name, paths::shm(), event_meta,
                                      event_queue, flow_dir, reader_name, cfg_path);
    } catch (const std::exception& e) {
        SPDLOG_CRITICAL("td api init failed | error=\"{}\"", e.what());
        return 1;
    }
    api->set_recover(recover);
    api->set_configs(std::move(td_cfg));
    ...
    SPDLOG_INFO("stop");
    g_event_sem = nullptr;  // 防止信号处理器在析构后访问悬垂指针
    spdlog::shutdown();
    return 0;
}
