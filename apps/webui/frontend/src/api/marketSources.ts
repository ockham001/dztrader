...
// 自动登录/登出排程请求体 (契约 auto-login: 全量提交, 直发 SET_AUTO_LOGIN)
...
// 订阅参数请求体 (契约 md-config SetSubscribeParams): 全部可选, 缺失=保留旧值, 无状态保护
...
// SHM 行情通道配置请求体 (契约 shm SET_MD_SHM_CONFIG): RFC 7386 递归合并 patch。
// preload_points 内 key 的 value 为 null = 删除该时间点 (契约 shm 唯一合法 null 位置);
...
export const marketSourcesApi = {
  ...
  // 自动登录/登出排程 (契约 auto-login): 全量提交 {enabled, schedules}, 直发 SET_AUTO_LOGIN 帧,
  ...
  // 修改订阅参数 (契约 md-config): 可选字段 patch, 直发 SET_MD_CONFIG{SetSubscribeParams},
  ...
  // 修改 SHM 行情通道配置 (契约 shm): merge patch 直发 SET_MD_SHM_CONFIG,
