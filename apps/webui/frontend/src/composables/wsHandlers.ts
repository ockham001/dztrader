...
registerHandler('snapshot', (payload) => {
  ...
  // 契约 10 §6: 快照分发后清进程类 pending（断连期间挂起的 start/stop/remove
...
registerHandler('process_status', (payload) => {
  ...
  // P6: event 字段（契约 03）承担进程操作 pending 清理 + Remove 成功卡片移除
  // (原 rtn_process_control 消息后端已不推送, 见 10-webui-ws.md §5 已知差异关闭)
...
registerHandler('process_config', (payload) => {
  ...
  // 全量映射 { [进程名]: config, ... }，整体覆盖（覆盖天然含删除，契约 03）
...
registerHandler('md_rtn_config', (payload) => {
  // dzmd_ctp 配置变化时推送 (已脱敏, 契约 08: brokers/current_broker_name/订阅参数)
...
registerHandler('auto_login', (payload, instanceId) => {
  // 契约 04: RTN_AUTO_LOGIN → WS auto_login 消息 (data: {enabled, schedules}, 始终全量)
...
registerHandler('progress', (payload, instanceId) => {
  // 契约 05 progress 镜像：单条完整状态，后到覆盖先到（P2 后端已推，前端接入）
