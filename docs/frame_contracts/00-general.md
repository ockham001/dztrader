...
| 编号 | 主题 | 覆盖范围 |
|------|------|----------|
| 00 | 通用规则（本文件） | 帧布局、传输语义、请求-响应总则、快照查询 |
| 01 | 日志 | SET/FLUSH/RTN_LOG_CONFIG |
| 02 | SHM 通道配置 | 事件/行情通道配置 + 预加载 + 订阅者刷新 |
| 03 | UI 通知 | NOTIFY_UI |
...
- 变长帧在固定头后紧跟一个扩展头，三选一：
  ...
  - `DzExtInstFrameHeader`：`instance_id`（`char[64]`）+ `data_size`，带实例路由。
  - `DzExtStgFrameHeader`：`strategy_id` + `data_size`，策略来源标识（策略帧专用）。
- 定长 struct 帧无扩展头。
...
| 10-25 | 系统帧 |
| 101-121 | UI 通知/输出、逻辑持仓、全量快照、进程、自动登录、进度 |
| 1000-1014 | 行情（数据推送 1000，控制/RTN 1001-1014） |
...
- SHM 写入任何帧都唤醒全部等待该通道的进程（NamedSemaphore）；接收方按 `frame_type` 与 `instance_id` 自行过滤。
- 事件通道（`event`）：多进程写，承载控制/配置/通知帧（本目录全部契约覆盖的帧除行情数据帧外均走事件通道）。
...
- **未覆盖**（后续独立契约，本目录暂不收录）：交易帧（除《帧契约：交易委托请求》已覆盖的 `TD_ORDER_REQ`/`TD_ORDER_CANCEL_REQ` 外，其余 2000-2114，TD 已实现大半）、策略帧（3001+，`OUTPUT_UI`/`SET_LOGICAL_POSITION` 等）、行情/交易数据帧（`RTN_MD_TICK`、TD 推送 2000-2004，struct payload）。
...
1. **覆盖帧**：帧名清单（帧号不写，见 §3）
2. **语义/方向/路由**：谁发给谁、`instance_id` 语义、广播或定向
