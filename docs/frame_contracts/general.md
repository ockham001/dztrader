...
- **未覆盖**（后续独立契约，本目录暂不收录）：交易帧（除《帧契约：交易委托请求》已覆盖的 `TD_ORDER_REQ`/`TD_ORDER_CANCEL_REQ` 与《帧契约：账户登录状态》已覆盖的 `ACCOUNT_STATUS`/`TD_QUERY_ACCOUNT_STATUS` 外，其余 2000-2115，TD 已实现大半）、行情/交易数据帧（`RTN_MD_TICK`、TD 推送 2000-2004，struct payload）。
