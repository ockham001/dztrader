...
网关的动态状态（连接/登录/断开等）与步骤进度由 `RTN_PROGRESS`（契约 06）覆盖，本帧不包含状态与进度字段。

---

## 接口类型识别

与契约 08（《行情网关配置》）完全一致：payload 格式由 `instance_id` 派生的 `interface_type` 决定，payload 中不携带类型字段。当前仅定义 CTP 类型。
