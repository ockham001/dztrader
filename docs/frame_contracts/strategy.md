...
- `SCHEDULE`：策略定时器触发帧（`dz_schedule_*`），**仅 SDK 本地合成、不写共享内存**，经 `dz_next_event` 返回；basic 布局 `DzFrameHeader` + `DzScheduleEvent{timer_id}`（16 字节），指针有效期至下一次 `dz_next_event`/`dz_release`。
...
**页大小约束**：本契约两个变长帧（`UI_INPUT`/`OUTPUT_UI`）的总大小（帧头+扩展头+payload）不得超过通道单页大小；`OUTPUT_UI` 侧由策略 SDK 写侧（`dz_output_ui`）按 `min(1MB, 页大小−帧头开销)` 截断 payload，超出部分丢弃；`UI_INPUT` 侧 UI 投递方须遵循同一单页上限。
...
- 策略侧接收 API 由 SDK 提供（`on_ui_input` 回调，见 `strategy_base.h`）；SDK 按 `instance_id` == 本策略名 定向过滤后再回调
