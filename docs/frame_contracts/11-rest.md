...
REST 是**请求入口与大数据查询**；状态推送一律走 WS（契约 10）。REST 端点是总则 §4.2 形态 1/2/3 的前端入口。设置类请求的生效信号是 WS 领域消息（RTN 推送），REST 响应只表示"请求已被 dzweb 处理"。
...
- 数据变更联动：变更 DB 的端点成功后由 dzweb 广播 `data_changed{scope}`（契约 10），前端按 scope 重新 REST 拉取。
...
| DELETE `/api/market-sources/{id}` | 移除进程条目 | → `REQUEST_PROCESS_CONTROL{Remove}`（契约 04）；DB 主表记录保留（历史/审计），进程配置条目消失 = 移除完成的权威信号 |
| POST `/api/market-sources/{id}/login` | 行情连接请求 | → `REQUEST_MD_CONNECT`（`instance_id` = 进程名）；生效信号：`progress`/健康度（契约 07） |
| POST `/api/market-sources/{id}/logout` | 行情断开请求 | → `REQUEST_MD_DISCONNECT`；生效信号同上 |
| PUT `/api/market-sources/{id}/auto-login` | 全量设置自动登录排程 `{enabled, schedules}` | → `SET_AUTO_LOGIN`；生效信号：`auto_login` 推送（契约 05） |
| POST `/api/market-sources/{id}/brokers` | 添加经纪商 | → `SET_MD_CONFIG{AddBroker}`；生效信号：`md_rtn_config`（契约 08） |
...
| POST `/api/logs/level` | 设置日志级别 | → `SET_LOG_CONFIG`；dzweb 自身直调；生效信号：`log_config` 推送（契约 01） |
| POST `/api/logs/flush` | flush 日志缓冲 | → `FLUSH_LOG`；无反馈（契约 01） |

### 2.6 通知

| 方法/路径 | 语义 |
|---|---|
| GET `/api/notifications` | 最近通知（读取 NotifyCache，契约 03） |

---

## 3. 真相源声明

- **行情源列表**：DB（repository）为真相源。行情进程上报 `RTN_MD_CONFIG` 时，dzweb 对未入库的 `dzmd_*` 来源自动对账补录（幂等）。DB 与 master 进程配置（契约 04）的同步通过帧联动实现：添加行情源 + Start 由 master 动态注册（契约 04）。
- **进程存在性与运行状态**：master 进程配置为真相源（契约 04）；REST 列表是投影。
...
- 设置类 REST 请求成功后进入 pending，以对应 WS 领域消息清除（契约 10 §5）；REST 响应成功≠已生效。
